...
`)}
`).replace("<<type>>",isTypescript?": StorybookConfig":"").replace("<<mainContents>>",mainContents);await import_fs_extra3.default.writeFile(`./${storybookConfigFolder}/main.${isTypescript?"ts":"js"}`,(0,import_ts_dedent.dedent)(mainJsContents),{encoding:"utf8"})}async function configurePreview(options){let{prefix=""}=options.frameworkPreviewParts||{},isTypescript=options.language==="typescript-4-9"||options.language==="typescript-3-8",previewPath=`./${options.storybookConfigFolder}/preview.${isTypescript?"ts":"js"}`;if(await import_fs_extra3.default.pathExists(previewPath))return;let preview=import_ts_dedent.dedent`
    ${prefix}
...
`.trimStart(),promptForCompoDocs=async()=>{let{useCompoDoc}=await(0,import_prompts2.default)({type:"confirm",name:"useCompoDoc",message:"Do you want to use Compodoc for documentation?"});return useCompoDoc},AngularJSON=class{constructor(){if(!import_fs4.default.existsSync(ANGULAR_JSON_PATH))throw commandLog("An angular.json file was not found in the current directory. Storybook needs it to work properly."),new Error("No angular.json file found");let jsonContent=import_fs4.default.readFileSync(ANGULAR_JSON_PATH,"utf8");this.json=JSON.parse(jsonContent)}get projects(){return this.json.projects}get projectsWithoutStorybook(){return Object.keys(this.projects).filter(projectName=>{let{architect}=this.projects[projectName];return!architect.storybook})}getProjectSettingsByName(projectName){return this.projects[projectName]}async getProjectName(){if(this.projectsWithoutStorybook.length>1){let{projectName}=await(0,import_prompts2.default)({type:"select",name:"projectName",message:"For which project do you want to generate Storybook configuration?",choices:this.projectsWithoutStorybook.map(name=>({title:name,value:name}))});return projectName}return this.projectsWithoutStorybook[0]}addStorybookEntries({angularProjectName,storybookFolder,useCompodoc,root}){let{architect}=this.projects[angularProjectName],baseOptions={configDir:storybookFolder,browserTarget:`${angularProjectName}:build`,compodoc:useCompodoc,...useCompodoc&&{compodocArgs:["-e","json","-d",root||"."]}};architect.storybook||(architect.storybook={builder:"@storybook/angular:start-storybook",options:{...baseOptions,port:6006}}),architect["build-storybook"]||(architect["build-storybook"]={builder:"@storybook/angular:build-storybook",options:{...baseOptions,outputDir:Object.keys(this.projects).length===1?"storybook-static":`dist/storybook/${angularProjectName}`}})}write(){import_fs4.default.writeFileSync(ANGULAR_JSON_PATH,JSON.stringify(this.json,null,2))}};var generator=async(packageManager,npmOptions,options,commandOptions)=>{var _a,_b;let packageJson=packageManager.retrievePackageJson(),angularVersionFromDependencies=(_a=import_semver5.default.coerce(packageManager.retrievePackageJson().dependencies["@angular/core"]))==null?void 0:_a.version,angularVersionFromDevDependencies=(_b=import_semver5.default.coerce(packageManager.retrievePackageJson().devDependencies["@angular/core"]))==null?void 0:_b.version,angularVersion=angularVersionFromDependencies||angularVersionFromDevDependencies,updatedOptions=import_semver5.default.gte(angularVersion,"12.0.0")?{...options,builder:"webpack5"}:options,angularJSON=new AngularJSON;if(angularJSON.projectsWithoutStorybook.length===0)return paddedLog("Every project in your workspace is already set up with Storybook. There is nothing to do!"),Promise.reject();let angularProjectName=await angularJSON.getProjectName();paddedLog(`Adding Storybook support to your "${angularProjectName}" project`);let{root}=angularJSON.getProjectSettingsByName(angularProjectName),{projects}=angularJSON,useCompodoc=commandOptions.yes?!0:await promptForCompoDocs(),storybookFolder=root?`${root}/.storybook`:".storybook";if(root!==""){import_fs5.default.mkdirSync(storybookFolder,{recursive:!0});let rootReferencePathFromStorybookFolder=root.split("/").map(()=>"../").join("");import_fs5.default.writeFileSync(`${storybookFolder}/main.ts`,(0,import_ts_dedent4.default)(`
    ...
    `},async run(options){return webpack5.run({...options,result:{webpackVersion:null,...options.result}})}};var import_chalk8=__toESM(require("chalk")),import_ts_dedent9=require("ts-dedent"),import_csf_tools2=require("@storybook/csf-tools"),import_core_common2=require("@storybook/core-common"),import_fs_extra7=require("fs-extra"),import_detect_indent=__toESM(require("detect-indent"));var import_fs_extra6=__toESM(require("fs-extra")),SUPPORTED_ESLINT_EXTENSIONS=["js","cjs","json"],UNSUPPORTED_ESLINT_EXTENSIONS=["yaml","yml"],findEslintFile=()=>{let filePrefix=".eslintrc",unsupportedExtension=UNSUPPORTED_ESLINT_EXTENSIONS.find(ext=>import_fs_extra6.default.existsSync(`${filePrefix}.${ext}`));if(unsupportedExtension)throw new Error(unsupportedExtension);let extension=SUPPORTED_ESLINT_EXTENSIONS.find(ext=>import_fs_extra6.default.existsSync(`${filePrefix}.${ext}`));return extension?`${filePrefix}.${extension}`:null};var logger7=console,eslintPlugin={id:"eslintPlugin",async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),{dependencies,devDependencies}=packageJson,eslintPluginStorybook=dependencies["eslint-plugin-storybook"]||devDependencies["eslint-plugin-storybook"],eslintDependency=dependencies.eslint||devDependencies.eslint;if(eslintPluginStorybook||!eslintDependency)return null;let{mainConfig}=(0,import_core_common2.getStorybookInfo)(packageJson);if(!mainConfig)return logger7.warn("Unable to find storybook main.js config, skipping"),null;let eslintFile,unsupportedExtension;try{eslintFile=findEslintFile()}catch(err){unsupportedExtension=err.message}if(!eslintFile&&!unsupportedExtension)return logger7.warn("Unable to find .eslintrc config file, skipping"),null;let main=await(0,import_csf_tools2.readConfig)(mainConfig);return{eslintFile,main,unsupportedExtension}},prompt(){return import_ts_dedent9.dedent`
      ...
      More info: ${import_chalk8.default.yellow("https://github.com/storybookjs/eslint-plugin-storybook#readme")}
    `},async run({result:{eslintFile,unsupportedExtension},packageManager,dryRun}){let deps=["eslint-plugin-storybook"];if(logger7.info(`\u2705 Adding dependencies: ${deps}`),dryRun||packageManager.addDependencies({installAsDevDependencies:!0},deps),!dryRun&&unsupportedExtension){logger7.info(import_ts_dedent9.dedent`
      ...
      `);return}if(logger7.info(`\u2705 Adding Storybook plugin to ${eslintFile}`),!dryRun)if(eslintFile.endsWith("json")){let eslintConfig=await(0,import_fs_extra7.readJson)(eslintFile),existingConfigValue=Array.isArray(eslintConfig.extends)?eslintConfig.extends:[eslintConfig.extends];eslintConfig.extends=[...existingConfigValue||[],"plugin:storybook/recommended"];let eslintFileContents=await(0,import_fs_extra7.readFile)(eslintFile,"utf8"),spaces=(0,import_detect_indent.default)(eslintFileContents).amount||2;await(0,import_fs_extra7.writeJson)(eslintFile,eslintConfig,{spaces})}else{let eslint=await(0,import_csf_tools2.readConfig)(eslintFile),extendsConfig=eslint.getFieldValue(["extends"])||[],existingConfigValue=Array.isArray(extendsConfig)?extendsConfig:[extendsConfig];eslint.setFieldValue(["extends"],[...existingConfigValue,"plugin:storybook/recommended"]),await(0,import_csf_tools2.writeConfig)(eslint)}}};var import_chalk9=__toESM(require("chalk")),import_ts_dedent10=require("ts-dedent"),import_csf_tools3=require("@storybook/csf-tools"),import_core_common3=require("@storybook/core-common"),logger8=console,builderVite={id:"builder-vite",async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),{mainConfig}=(0,import_core_common3.getStorybookInfo)(packageJson);if(!mainConfig)return logger8.warn("Unable to find storybook main.js config"),null;let main=await(0,import_csf_tools3.readConfig)(mainConfig),builder=main.getFieldValue(["core","builder"]);return(typeof builder=="string"?builder:builder==null?void 0:builder.name)!=="storybook-builder-vite"?null:{builder,main,packageJson}},prompt({builder}){let builderFormatted=import_chalk9.default.cyan(JSON.stringify(builder,null,2));return import_ts_dedent10.dedent`
      We've detected you're using the community vite builder: ${builderFormatted}
      
      'storybook-builder-vite' is deprecated and now located at ${import_chalk9.default.cyan("@storybook/builder-vite")}.

      We can upgrade your project to use the new builder automatically.
      
      More info: ${import_chalk9.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#vite-builder-renamed")}
    `},async run({result:{builder,main,packageJson},packageManager,dryRun}){let{dependencies={},devDependencies={}}=packageJson;if(logger8.info("Removing existing 'storybook-builder-vite' dependency"),dryRun||(delete dependencies["storybook-builder-vite"],delete devDependencies["storybook-builder-vite"],packageManager.writePackageJson(packageJson)),logger8.info("Adding '@storybook/builder-vite' as dev dependency"),dryRun||packageManager.addDependencies({installAsDevDependencies:!0},["@storybook/builder-vite"]),logger8.info("Updating main.js to use vite builder"),!dryRun){let updatedBuilder=typeof builder=="string"?"@storybook/builder-vite":{name:"@storybook/builder-vite",options:builder.options};main.setFieldValue(["core","builder"],updatedBuilder),await(0,import_csf_tools3.writeConfig)(main)}}};var import_chalk10=__toESM(require("chalk")),import_ts_dedent11=require("ts-dedent"),import_semver11=__toESM(require("semver")),import_core_common4=require("@storybook/core-common"),logger9=console,getStorybookScripts=allScripts=>Object.keys(allScripts).reduce((acc,key)=>{let isStorybookScript=!1,allWordsFromScript=allScripts[key].split(" "),newScript=allWordsFromScript.map((currentWord,index)=>{let previousWord=allWordsFromScript[index-1],isSbBinary=currentWord==="build-storybook"||currentWord==="start-storybook"||currentWord==="sb",isPrependedByPkgManager=previousWord&&["npx","run","yarn","pnpx"].some(cmd=>previousWord.includes(cmd));return isSbBinary&&!isPrependedByPkgManager?(isStorybookScript=!0,currentWord.replace("sb","storybook").replace("start-storybook","storybook dev").replace("build-storybook","storybook build")):currentWord}).join(" ");return isStorybookScript&&(acc[key]={before:allScripts[key],after:newScript}),acc},{}),sbScripts={id:"sb-scripts",async check({packageManager}){var _a;let packageJson=packageManager.retrievePackageJson(),{scripts={}}=packageJson,{version:storybookVersion}=(0,import_core_common4.getStorybookInfo)(packageJson),storybookCoerced=storybookVersion&&((_a=import_semver11.default.coerce(storybookVersion))==null?void 0:_a.version);if(!storybookCoerced)throw new Error(import_ts_dedent11.dedent`
        ❌ Unable to determine storybook version.
        🤔 Are you running automigrate from your project directory? Please specify your Storybook config directory with the --config-dir flag.
      `);if(import_semver11.default.lt(storybookCoerced,"7.0.0"))return null;let storybookScripts=getStorybookScripts(scripts);return Object.keys(storybookScripts).length===0?null:{packageJson,storybookScripts,storybookVersion}},prompt({storybookVersion,storybookScripts}){let sbFormatted=import_chalk10.default.cyan(`Storybook ${storybookVersion}`),newScriptsMessage=Object.keys(storybookScripts).reduce((acc,scriptKey)=>(acc.push([import_chalk10.default.bold(scriptKey),"from:",import_chalk10.default.cyan(storybookScripts[scriptKey].before),"to:",import_chalk10.default.cyan(storybookScripts[scriptKey].after)].join(`
`)),acc),[]);return import_ts_dedent11.dedent`
      We've detected you are using ${sbFormatted} with scripts from previous versions of Storybook.
      Starting in Storybook 7, the ${import_chalk10.default.yellow("start-storybook")} and ${import_chalk10.default.yellow("build-storybook")} binaries have changed to ${import_chalk10.default.magenta("storybook dev")} and ${import_chalk10.default.magenta("storybook build")} respectively.
...
`)}
      ...
      In case this migration did not cover all of your scripts, or you'd like more info: ${import_chalk10.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#start-storybook--build-storybook-binaries-removed")}
      `},async run({result:{storybookScripts},packageManager,dryRun}){if(logger9.info("\u2705 Updating scripts in package.json"),logger9.log(),!dryRun){let newScripts=Object.keys(storybookScripts).reduce((acc,scriptKey)=>(acc[scriptKey]=storybookScripts[scriptKey].after,acc),{});logger9.log(),packageManager.addScripts(newScripts)}}};var import_chalk11=__toESM(require("chalk")),import_ts_dedent12=require("ts-dedent"),import_semver12=__toESM(require("semver")),import_core_common5=require("@storybook/core-common");var logger10=console,sbBinary={id:"storybook-binary",async check({packageManager}){var _a;let packageJson=packageManager.retrievePackageJson(),{devDependencies,dependencies}=packageJson,{version:storybookVersion}=(0,import_core_common5.getStorybookInfo)(packageJson),allDeps={...dependencies,...devDependencies},storybookCoerced=storybookVersion&&((_a=import_semver12.default.coerce(storybookVersion))==null?void 0:_a.version);if(!storybookCoerced)throw new Error(import_ts_dedent12.dedent`
        ❌ Unable to determine storybook version.
        🤔 Are you running automigrate from your project directory? Please specify your Storybook config directory with the --config-dir flag.
      `);if(allDeps["@nrwl/storybook"]||import_semver12.default.lt(storybookCoerced,"7.0.0"))return null;let hasSbBinary=!!allDeps.sb,hasStorybookBinary=!!allDeps.storybook;return!hasSbBinary&&hasStorybookBinary?null:{hasSbBinary,hasStorybookBinary,storybookVersion,packageJson}},prompt({storybookVersion,hasSbBinary,hasStorybookBinary}){let sbFormatted=import_chalk11.default.cyan(`Storybook ${storybookVersion}`),storybookBinaryMessage=hasStorybookBinary?"":`We've detected you are using ${sbFormatted} without Storybook's ${import_chalk11.default.magenta("storybook")} binary. Starting in Storybook 7.0, it has to be installed.`;return import_ts_dedent12.dedent`
...
`:""}
      ...
      More info: ${import_chalk11.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#start-storybook--build-storybook-binaries-removed")}
      `},async run({result:{packageJson,hasSbBinary,hasStorybookBinary},packageManager,dryRun,skipInstall}){if(hasSbBinary&&(logger10.info("\u2705 Removing 'sb' dependency"),dryRun||packageManager.removeDependencies({skipInstall:skipInstall||!hasStorybookBinary,packageJson},["sb"])),!hasStorybookBinary&&(logger10.log(),logger10.info("\u2705 Adding 'storybook' as dev dependency"),logger10.log(),!dryRun)){let versionToInstall=getStorybookVersionSpecifier(packageJson);packageManager.addDependencies({installAsDevDependencies:!0,packageJson,skipInstall},[`storybook@${versionToInstall}`])}}};var import_chalk13=__toESM(require("chalk")),import_ts_dedent14=__toESM(require("ts-dedent")),import_find_up4=__toESM(require("find-up")),import_semver14=__toESM(require("semver")),import_core_common8=require("@storybook/core-common");var import_core_common6=require("@storybook/core-common"),import_prompts3=__toESM(require("prompts")),detectRenderer=async packageJson=>{let allDependencies={...packageJson.dependencies,...packageJson.devDependencies,...packageJson.peerDependencies},matches=Object.keys(import_core_common6.rendererPackages).filter(pkg2=>!!allDependencies[pkg2]),[rendererPackage]=matches;return matches.length>1&&(rendererPackage=(await(0,import_prompts3.default)([{type:"select",name:"rendererPackage",hint:"> - Use arrow-keys. Return to submit. To automate this next time, you can pass the renderer to the CLI via the --renderer flag",message:"Tried to detect a Storybook renderer in your project but found multiple. This could happen in monorepos, when projects contain multiple Storybook packages in package.json. Please select the correct one:",choices:matches.map(type=>({title:type,value:type}))}])).rendererPackage),rendererPackage};var import_find_up3=__toESM(require("find-up")),logger11=console,packagesMap={"@storybook/react":{webpack5:"@storybook/react-webpack5",vite:"@storybook/react-vite"},"@storybook/preact":{webpack5:"@storybook/preact-webpack5",vite:"@storybook/preact-vite"},"@storybook/server":{webpack5:"@storybook/server-webpack5"},"@storybook/ember":{webpack5:"@storybook/ember"},"@storybook/angular":{webpack5:"@storybook/angular"},"@storybook/vue":{webpack5:"@storybook/vue-webpack5",vite:"@storybook/vue-vite"},"@storybook/vue3":{webpack5:"@storybook/vue3-webpack5",vite:"@storybook/vue3-vite"},"@storybook/svelte":{webpack5:"@storybook/svelte-webpack5",vite:"@storybook/svelte-vite"},"@storybook/web-components":{webpack5:"@storybook/web-components-webpack5",vite:"@storybook/web-components-vite"},"@storybook/html":{webpack5:"@storybook/html-webpack5",vite:"@storybook/html-vite"}},communityFrameworks={vite:["storybook-framework-qwik","storybook-solidjs-vite"],webpack5:[]},viteConfigFiles2=["vite.config.js","vite.config.cjs","vite.config.mjs","vite.config.ts"],webpackConfigFiles=["webpack.config.js","webpack.config.cjs","webpack.config.mjs","webpack.config.ts"],detectBuilderInfo=async({mainConfig,configDir,packageDependencies})=>{var _a;let builderOptions={},builderName,builderOrFrameworkName,{core={},framework}=mainConfig,{builder}=core;if(builder?typeof builder=="string"?builderOrFrameworkName=builder:(builderOrFrameworkName=builder.name,builderOptions=builder.options||{}):framework&&(typeof framework=="string"?builderOrFrameworkName=framework:(builderOrFrameworkName=framework.name,builderOptions=((_a=framework.options)==null?void 0:_a.builder)||{})),!builderOrFrameworkName){let viteConfigFile=await(0,import_find_up3.default)(viteConfigFiles2,{cwd:configDir});if(viteConfigFile)logger11.info(`No builder or framework field, detected Storybook builder via: ${viteConfigFile}`),builderOrFrameworkName="vite";else{let webpackConfigFile=await(0,import_find_up3.default)(webpackConfigFiles,{cwd:configDir});webpackConfigFile&&(logger11.info(`No builder or framework field, detected Storybook builder via: ${webpackConfigFile}`),builderOrFrameworkName="webpack5")}}return builderOrFrameworkName||(packageDependencies["@storybook/builder-vite"]||packageDependencies["storybook-builder-vite"]?builderOrFrameworkName="vite":(packageDependencies["@storybook/builder-webpack5"]||packageDependencies["@storybook/manager-webpack5"])&&(builderOrFrameworkName="webpack5")),builderOrFrameworkName!=null&&builderOrFrameworkName.includes("vite")||communityFrameworks.vite.includes(builderOrFrameworkName)?builderName="vite":(builderOrFrameworkName!=null&&builderOrFrameworkName.includes("webpack")||communityFrameworks.webpack5.includes(builderOrFrameworkName),builderName="webpack5"),{name:builderName,options:builderOptions}},getNextjsAddonOptions=addons=>{let nextjsAddon=addons==null?void 0:addons.find(addon=>typeof addon=="string"?addon==="storybook-addon-next":addon.name==="storybook-addon-next");return!nextjsAddon||typeof nextjsAddon=="string"?{}:nextjsAddon.options||{}};var import_core_common7=require("@storybook/core-common"),import_csf_tools4=require("@storybook/csf-tools"),import_chalk12=__toESM(require("chalk")),import_semver13=__toESM(require("semver")),import_ts_dedent13=__toESM(require("ts-dedent")),logger12=console,getStorybookData=async({packageManager,configDir:userDefinedConfigDir})=>{var _a;let packageJson=packageManager.retrievePackageJson(),{mainConfig:mainConfigPath,version:storybookVersionSpecifier,configDir:configDirFromScript}=(0,import_core_common7.getStorybookInfo)(packageJson,userDefinedConfigDir),storybookVersion=storybookVersionSpecifier&&((_a=import_semver13.default.coerce(storybookVersionSpecifier))==null?void 0:_a.version),configDir=userDefinedConfigDir||configDirFromScript||".storybook",mainConfig;try{mainConfig=await(0,import_core_common7.loadMainConfig)({configDir})}catch(err){throw new Error(import_ts_dedent13.default`Unable to find or evaluate ${import_chalk12.default.blue(mainConfigPath)}: ${err.message}`)}return{configDir,mainConfig,storybookVersionSpecifier,storybookVersion,mainConfigPath}},updateMainConfig=async({mainConfigPath,dryRun},callback)=>{try{let main=await(0,import_csf_tools4.readConfig)(mainConfigPath);await callback(main),dryRun||await(0,import_csf_tools4.writeConfig)(main)}catch(e){logger12.info(`\u274C The migration failed to update your ${import_chalk12.default.blue(mainConfigPath)} on your behalf because of the following error:
        ${e}
`),logger12.info(`\u26A0\uFE0F Storybook automigrations are based on AST parsing and it's possible that your ${import_chalk12.default.blue(mainConfigPath)} file contains a non-standard format (e.g. your export is not an object) or that there was an error when parsing dynamic values (e.g. "require" calls, or usage of environment variables). When your main config is non-standard, automigrations are unfortunately not possible. Please follow the instructions given previously and follow the documentation to make the updates manually.`)}};var logger13=console,nextJsConfigFiles=["next.config.js","next.config.cjs","next.config.mjs","next.config.ts"],newFrameworks={id:"new-frameworks",async check({rendererPackage:userDefinedRendererPackage,configDir:userDefinedConfigDir,packageManager}){var _a,_b,_c;let packageJson=packageManager.retrievePackageJson(),{storybookVersion,mainConfig,mainConfigPath,configDir}=await getStorybookData({packageManager,configDir:userDefinedConfigDir});if(!import_semver14.default.gte(storybookVersion,"7.0.0"))return null;let frameworkPackage=typeof mainConfig.framework=="string"?mainConfig.framework:(_a=mainConfig.framework)==null?void 0:_a.name,hasFrameworkInMainConfig=!!frameworkPackage,rendererPackage=userDefinedRendererPackage;rendererPackage||(frameworkPackage&&Object.keys(import_core_common8.rendererPackages).includes(frameworkPackage)?rendererPackage=frameworkPackage:frameworkPackage&&Object.values(import_core_common8.rendererPackages).includes(frameworkPackage)?(rendererPackage=Object.keys(import_core_common8.rendererPackages).find(k=>import_core_common8.rendererPackages[k]===frameworkPackage),hasFrameworkInMainConfig=!1):rendererPackage=await detectRenderer(packageJson));let builderConfig=(_b=mainConfig.core)==null?void 0:_b.builder;if(!Object.keys(packagesMap).includes(rendererPackage))return null;let allDependencies=packageManager.getAllDependencies(),builderInfo=await detectBuilderInfo({mainConfig,configDir,packageDependencies:allDependencies}),newFrameworkPackage=Object.keys(import_core_common8.frameworkPackages).find(pkg2=>pkg2===frameworkPackage);if(newFrameworkPackage||(newFrameworkPackage=packagesMap[rendererPackage]&&packagesMap[rendererPackage][builderInfo.name]),!newFrameworkPackage)return null;let renderer=import_core_common8.rendererPackages[rendererPackage],rendererOptions=mainConfig[`${renderer}Options`]||{},frameworkOptions=typeof mainConfig.framework=="string"?{}:(_c=mainConfig.framework)==null?void 0:_c.options,dependenciesToRemove=["@storybook/builder-webpack5","@storybook/manager-webpack5","@storybook/builder-webpack4","@storybook/manager-webpack4","@storybook/builder-vite","storybook-builder-vite"],addonsToRemove=[],addonOptions={},metaFramework;if(rendererPackage==="@storybook/react"&&allDependencies.next){let nextConfigFile=await(0,import_find_up4.default)(nextJsConfigFiles,{cwd:configDir}),nextAddonOptions=getNextjsAddonOptions(mainConfig.addons);(import_semver14.default.gte(import_semver14.default.coerce(allDependencies.next).version,"12.0.0")&&nextConfigFile||Object.keys(nextAddonOptions).length>0)&&(metaFramework="nextjs",(newFrameworkPackage==="@storybook/react-webpack5"||newFrameworkPackage==="@storybook/nextjs")&&(newFrameworkPackage="@storybook/nextjs",addonsToRemove=["storybook-addon-next","storybook-addon-next-router"].filter(dep=>{var _a2;return allDependencies[dep]||((_a2=mainConfig.addons)==null?void 0:_a2.find(addon=>typeof addon=="string"?dep===addon:dep===addon.name))}),addonOptions=nextAddonOptions,dependenciesToRemove.push("@storybook/react-webpack5","storybook-addon-next","storybook-addon-next-router")))}else rendererPackage==="@storybook/svelte"&&allDependencies["@sveltejs/kit"]&&import_semver14.default.gte(import_semver14.default.coerce(allDependencies["@sveltejs/kit"]).version,"1.0.0")&&(metaFramework="svelte-kit",newFrameworkPackage==="@storybook/svelte-vite"&&(newFrameworkPackage="@storybook/svelte-kit",rendererOptions={},dependenciesToRemove.push("@storybook/svelte-vite")));let dependenciesToAdd=[newFrameworkPackage].filter(dep=>!allDependencies[dep]).filter(Boolean);if(dependenciesToRemove=dependenciesToRemove.filter(dep=>allDependencies[dep]).filter(Boolean),hasFrameworkInMainConfig&&!builderConfig&&!Object.keys(rendererOptions).length&&!Object.keys(addonOptions).length&&!dependenciesToRemove.length&&!dependenciesToAdd.length)return null;if(allDependencies.vite&&import_semver14.default.lt(import_semver14.default.coerce(allDependencies.vite).version,"3.0.0"))throw new Error(import_ts_dedent14.default`
        ❌ Your project should be upgraded to use the framework package ${import_chalk13.default.bold(newFrameworkPackage)}, but we detected that you are using Vite ${import_chalk13.default.bold(allDependencies.vite)}, which is unsupported in ${import_chalk13.default.bold("Storybook 7.0")}. Please upgrade Vite to ${import_chalk13.default.bold("3.0.0 or higher")} and rerun this migration.
      ...
      ${dependenciesToRemove.map(dep=>`- * ${import_chalk13.default.cyan(dep)}`).join(`
`)}
`),dependenciesToAdd.length>0&&(migrationSteps+=`- Add the following dependencies: 
      ${dependenciesToAdd.map(dep=>`- * ${import_chalk13.default.cyan(dep)}`).join(`
`)}
`),hasFrameworkInMainConfig||(migrationSteps+=`- Update or specify the ${import_chalk13.default.yellow("framework")} field in ${import_chalk13.default.blue(mainConfigPath)} with the value of "${import_chalk13.default.cyan(frameworkPackage)}".
`),Object.keys(rendererOptions).length>0&&(migrationSteps+=`- Move the ${import_chalk13.default.yellow(`${renderer}Options`)} field in ${import_chalk13.default.blue(mainConfigPath)} to ${import_chalk13.default.yellow("framework.options")}, and remove that field entirely.
      More info: ${import_chalk13.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#frameworkoptions-renamed")}
`),addonsToRemove.length>0&&(migrationSteps+=`- Remove the following addons from your ${import_chalk13.default.blue(mainConfigPath)}, as the new framework also supports features provided by them:
      ${addonsToRemove.map(dep=>`- * ${import_chalk13.default.cyan(dep)}`).join(`
`)}
      `),Object.keys(addonOptions).length>0&&(migrationSteps+=`- Move the addon options "${import_chalk13.default.yellow(Object.keys(addonOptions).join(", "))}" in ${import_chalk13.default.blue(mainConfigPath)} to the ${import_chalk13.default.yellow("framework.options")} field.
`),builderConfig&&(typeof builderConfig=="object"&&Object.keys(builderConfig.options||{}).length>0&&(migrationSteps+=`- Move the ${import_chalk13.default.yellow("core.builder.options")} field in ${import_chalk13.default.blue(mainConfigPath)} to ${import_chalk13.default.yellow("framework.options.builder")}
`),migrationSteps+=`- Remove the ${import_chalk13.default.yellow("core.builder")} field in ${import_chalk13.default.blue(mainConfigPath)}.
`),(dependenciesToRemove.includes("@storybook/builder-webpack4")||dependenciesToRemove.includes("@storybook/manager-webpack4"))&&(disclaimer=import_ts_dedent14.default`\n\n
      ${import_chalk13.default.underline(import_chalk13.default.bold(import_chalk13.default.cyan("Webpack 4 users")))}
      ...
      To learn more about migrating from Webpack4, see: ${import_chalk13.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#webpack4-support-discontinued")}`),metaFramework==="nextjs"&&(dependenciesToRemove.includes("storybook-addon-next-router")&&(migrationSteps+=`- Migrate the usage of the ${import_chalk13.default.cyan("storybook-addon-next-router")} addon to use the APIs from the ${import_chalk13.default.magenta("@storybook/nextjs")} framework package instead. Follow the instructions below.`),frameworkPackage==="@storybook/react-vite"?disclaimer=import_ts_dedent14.default`\n\n
          ${import_chalk13.default.bold("Important")}: We've detected you are using Storybook in a Next.js project.
  
          This migration is set to update your project to use the ${import_chalk13.default.magenta("@storybook/react-vite")} framework, but Storybook provides a framework package specifically for Next.js projects: ${import_chalk13.default.magenta("@storybook/nextjs")}.
        ...
        `:frameworkPackage==="@storybook/nextjs"&&(disclaimer=import_ts_dedent14.default`\n\n
        The ${import_chalk13.default.magenta("@storybook/nextjs")} package provides great user experience for Next.js users, and we highly recommend you to read more about it at ${import_chalk13.default.yellow("https://github.com/storybookjs/storybook/blob/next/code/frameworks/nextjs/README.md")}
        `)),metaFramework==="svelte-kit"&&(frameworkPackage==="@storybook/svelte-webpack5"?disclaimer=import_ts_dedent14.default`\n\n
          ${import_chalk13.default.bold("Important")}: We've detected you are using Storybook in a Svelte kit project.
  
          This migration is set to update your project to use the ${import_chalk13.default.magenta("@storybook/svelte-webpack5")} framework, but Storybook provides a framework package specifically for Svelte kit projects: ${import_chalk13.default.magenta("@storybook/sveltekit")}.
          ...
          If you are interested in using this package, see: ${import_chalk13.default.yellow("https://github.com/storybookjs/storybook/blob/next/code/frameworks/sveltekit/README.md")}
        `:disclaimer=import_ts_dedent14.default`\n\n
        The ${import_chalk13.default.magenta("@storybook/sveltekit")} package provides great user experience for Svelte kit users, and we highly recommend you to read more about it at ${import_chalk13.default.yellow("https://github.com/storybookjs/storybook/blob/next/code/frameworks/sveltekit/README.md")}
...
`)}
    ...
    `}};var import_chalk15=__toESM(require("chalk")),import_ts_dedent16=require("ts-dedent"),import_path11=require("path"),import_fs_extra9=__toESM(require("fs-extra")),import_globby=__toESM(require("globby")),MDX1_STYLE_START=/<style>{`/g,MDX1_STYLE_END=/`}<\/style>/g,MDX1_COMMENT=/<!--(.+)-->/g,MDX1_CODEBLOCK=/(?:\n~~~(?:\n|.)*?\n~~~)|(?:\n```(?:\n|.)*?\n```)/g,fixMdxStyleTags=mdx=>mdx.replace(MDX1_STYLE_START,"<style>\n  {`").replace(MDX1_STYLE_END,"  `}\n</style>"),fixMdxComments=mdx=>{let codeblocks=mdx.matchAll(MDX1_CODEBLOCK);return mdx.split(MDX1_CODEBLOCK).map(v=>v.replace(MDX1_COMMENT,(original,group)=>`{/*${group}*/}`)).reduce((acc,item,i)=>{let next=codeblocks.next();return next.done?acc+item:acc+item+next.value[0]},"")},logger14=console,mdx1to2={id:"mdx1to2",async check(){let storiesMdxFiles=await(0,import_globby.default)("./!(node_modules)**/*.(story|stories).mdx");return storiesMdxFiles.length?{storiesMdxFiles}:void 0},prompt({storiesMdxFiles}){return import_ts_dedent16.dedent`
      ...
      ${import_chalk15.default.cyan("https://storybook.js.org/migration-guides/7.0")}
    `},async run({result:{storiesMdxFiles},dryRun}){await Promise.all([...storiesMdxFiles.map(async fname=>{let contents=await import_fs_extra9.default.readFile(fname,"utf-8"),updated=fixMdxComments(fixMdxStyleTags(contents));updated===contents?logger14.info(`\u{1F197} Unmodified ${(0,import_path11.basename)(fname)}`):(logger14.info(`\u2705 Modified ${(0,import_path11.basename)(fname)}`),dryRun||await import_fs_extra9.default.writeFile(fname,updated))})])}};var import_chalk16=__toESM(require("chalk")),import_ts_dedent17=require("ts-dedent"),import_csf_tools5=require("@storybook/csf-tools"),import_core_common10=require("@storybook/core-common"),logger15=console,autodocsTrue={id:"autodocsTrue",async check({packageManager}){let packageJson=packageManager.retrievePackageJson(),{mainConfig}=(0,import_core_common10.getStorybookInfo)(packageJson);if(!mainConfig)return logger15.warn("Unable to find storybook main.js config, skipping"),null;let main=await(0,import_csf_tools5.readConfig)(mainConfig),docs=main.getFieldValue(["docs"]),docsPageToAutodocsMapping={true:"tag",automatic:!0,false:!1};if(docs!=null&&docs.docsPage){let oldValue=docs==null?void 0:docs.docsPage.toString();if(!(oldValue in docsPageToAutodocsMapping))throw new Error(`Unexpected value for docs.docsPage: ${oldValue}`);return{main,value:docsPageToAutodocsMapping[oldValue]}}return(docs==null?void 0:docs.autodocs)===void 0?{main}:null},prompt({value}){let autodocsFormatted=import_chalk16.default.cyan(`docs: { autodocs: ${JSON.stringify(value??!0)} }`);return value?import_ts_dedent17.dedent`
      ...
      More info: ${import_chalk16.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#autodocs")}
    `:import_ts_dedent17.dedent`
...
`);return import_ts_dedent18.dedent`
      ...
      More info: ${import_chalk17.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#react-peer-dependencies-required")}
    `},async run({packageManager,result:{additionalDependencies},dryRun}){dryRun||packageManager.addDependencies({installAsDevDependencies:!0},additionalDependencies)}};var import_chalk18=__toESM(require("chalk")),import_ts_dedent19=__toESM(require("ts-dedent")),import_semver15=__toESM(require("semver")),import_core_common11=require("@storybook/core-common"),nodeJsRequirement={id:"nodejs-requirement",promptOnly:!0,async check({packageManager}){var _a;let packageJson=packageManager.retrievePackageJson(),{version:storybookVersion}=(0,import_core_common11.getStorybookInfo)(packageJson),storybookCoerced=storybookVersion&&((_a=import_semver15.default.coerce(storybookVersion))==null?void 0:_a.version);if(!storybookCoerced)throw new Error(import_ts_dedent19.default`
        ❌ Unable to determine storybook version.
        🤔 Are you running automigrate from your project directory? Please specify your Storybook config directory with the --config-dir flag.
      `);if(!import_semver15.default.gte(storybookCoerced,"7.0.0"))return null;let nodeVersion=process.version;return import_semver15.default.lt(nodeVersion,"16.0.0")?{nodeVersion}:null},prompt({nodeVersion}){return import_ts_dedent19.default`
    ...
    `}};var import_chalk19=__toESM(require("chalk")),import_ts_dedent20=__toESM(require("ts-dedent")),import_semver16=__toESM(require("semver")),import_core_common12=require("@storybook/core-common"),import_core=require("@babel/core"),import_csf_tools6=require("@storybook/csf-tools");var logger16=console,frameworksThatNeedBabelConfig=["@storybook/react-webpack5","@storybook/vue-webpack5","@storybook/vue3-webpack5","@storybook/html-webpack5"],missingBabelRc={id:"missing-babelrc",async check({packageManager}){var _a;let packageJson=packageManager.retrievePackageJson(),{mainConfig,version:storybookVersion}=(0,import_core_common12.getStorybookInfo)(packageJson),storybookCoerced=storybookVersion&&((_a=import_semver16.default.coerce(storybookVersion))==null?void 0:_a.version);if(!storybookCoerced)throw new Error(import_ts_dedent20.default`
        ❌ Unable to determine storybook version.
        🤔 Are you running automigrate from your project directory? Please specify your Storybook config directory with the --config-dir flag.
      `);if(!import_semver16.default.gte(storybookCoerced,"7.0.0"))return null;if(!mainConfig)return logger16.warn("Unable to find storybook main.js config, skipping"),null;let main=await(0,import_csf_tools6.readConfig)(mainConfig),frameworkPackage=main.getNameFromPath(["framework"]),addons=main.getNamesFromPath(["addons"]),hasCraPreset=addons&&addons.find(addon=>addon==="@storybook/preset-create-react-app");if(frameworkPackage&&frameworksThatNeedBabelConfig.includes(frameworkPackage)&&!hasCraPreset){let config=await(0,import_core.loadPartialConfigAsync)({babelrc:!0,filename:"__fake__.js"});if(!config.config&&!config.babelrc&&!packageJson.babel)return{needsBabelRc:!0}}return null},prompt(){return import_ts_dedent20.default`
      We detected that your project does not have a babel configuration (.babelrc, babel.config.js, etc.).

      In version 6.x, Storybook provided its own babel settings out of the box. Now, Storybook re-uses ${import_chalk19.default.bold("your project's babel configuration")}, with small, incremental updates from Storybook addons.
      ...
      We can create a ${import_chalk19.default.blue(".babelrc.json")} file with some basic configuration and add any necessary package devDependencies.

      Please see the migration guide for more information:
      ${import_chalk19.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#babel-mode-v7-exclusively")}
    `},async run(){logger16.info(),await generateStorybookBabelConfigInCWD()}};var import_chalk20=__toESM(require("chalk")),import_ts_dedent21=__toESM(require("ts-dedent")),import_semver17=__toESM(require("semver")),import_csf_tools7=require("@storybook/csf-tools"),import_core_common13=require("@storybook/core-common"),logger17=console,fixId="bare-mdx-stories-glob",getNextGlob=glob=>{let extGlobsRegex=new RegExp(/(.*\.)(stories\.@.*)(\|mdx|mdx\|)(.*)$/i);if(glob.match(extGlobsRegex))return glob.replace(extGlobsRegex,"$1@(mdx|$2$4)");let allStoriesExtensionsRegex=new RegExp(/(.*\.)(stories\.\*)$/i);return glob.match(allStoriesExtensionsRegex)?glob.replace(allStoriesExtensionsRegex,"$1@(mdx|$2)"):glob.replaceAll(".stories.mdx",".mdx")},bareMdxStoriesGlob={id:fixId,async check({packageManager}){var _a;let packageJson=packageManager.retrievePackageJson(),{mainConfig,version:storybookVersion}=(0,import_core_common13.getStorybookInfo)(packageJson);if(!mainConfig)return logger17.warn("Unable to find storybook main.js config, skipping"),null;let sbVersionCoerced=storybookVersion&&((_a=import_semver17.default.coerce(storybookVersion))==null?void 0:_a.version);if(!sbVersionCoerced)throw new Error(import_ts_dedent21.default`
        ❌ Unable to determine storybook version.
        🤔 Are you running automigrate from your project directory? Please specify your Storybook config directory with the --config-dir flag.
      `);if(!import_semver17.default.gte(sbVersionCoerced,"7.0.0"))return null;let main=await(0,import_csf_tools7.readConfig)(mainConfig),existingStoriesEntries;try{existingStoriesEntries=main.getFieldValue(["stories"])}catch{}if(!existingStoriesEntries)throw new Error(import_ts_dedent21.default`
      ❌ Unable to determine Storybook stories globs, skipping ${import_chalk20.default.cyan(fixId)} fix.
      ...
      To learn more about this change, see: ${import_chalk20.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
...
`);return import_ts_dedent21.default`
    ...
    That would result in the following 'stories' config:
      ${import_chalk20.default.cyan(prettyNextStoriesEntries)}

    To learn more about this change, see: ${import_chalk20.default.yellow("https://github.com/storybookjs/storybook/blob/next/MIGRATION.md#mdx-docs-files")}
  `},async run({dryRun,result:{nextStoriesEntries,main}}){logger17.info(import_ts_dedent21.default`✅ Setting 'stories' config:
      ${JSON.stringify(nextStoriesEntries,null,2)}`),main.setFieldValue(["stories"],nextStoriesEntries),dryRun||await(0,import_csf_tools7.writeConfig)(main)}};var fixes=[nodeJsRequirement,cra5,webpack5,angular12,vue3,eslintPlugin,builderVite,sbBinary,sbScripts,newFrameworks,removedGlobalClientAPIs,mdx1to2,bareMdxStoriesGlob,autodocsTrue,addReact,missingBabelRc];var import_os=require("os"),ansiRegex=({onlyFirst=!1}={})=>{let pattern=["[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]+)*|[a-zA-Z\\d]+(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]*)*)?\\u0007)","(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PR-TZcf-ntqry=><~]))"].join("|");return new RegExp(pattern,onlyFirst?void 0:"g")},cleanLog=str=>str.replace(ansiRegex(),"").replace(/╮│/g,`\u256E
\u2502`).replace(/││/g,`\u2502
\u2502`).replace(/│╰/g,`\u2502
\u2570`).replace(/⚠️ {2}failed to check/g,`${import_os.EOL}\u26A0\uFE0F  failed to check`);var logger18=console,LOG_FILE_NAME="migration-storybook.log",LOG_FILE_PATH=(0,import_path12.join)(process.cwd(),LOG_FILE_NAME),TEMP_LOG_FILE_PATH="",originalStdOutWrite=process.stdout.write.bind(process.stdout),originalStdErrWrite=process.stderr.write.bind(process.stdout),augmentLogsToFile=()=>{TEMP_LOG_FILE_PATH=import_tempy.default.file({name:LOG_FILE_NAME});let logStream=(0,import_fs_extra10.createWriteStream)(TEMP_LOG_FILE_PATH);process.stdout.write=d=>(originalStdOutWrite(d),logStream.write(cleanLog(d))),process.stderr.write=d=>logStream.write(cleanLog(d))},cleanup=()=>{process.stdout.write=originalStdOutWrite,process.stderr.write=originalStdErrWrite};var logAvailableMigrations=()=>{let availableFixes=fixes.map(f=>import_chalk21.default.yellow(f.id)).join(", ");logger18.info(`
The following migrations are available: ${availableFixes}`)},automigrate=async({fixId:fixId2,dryRun,yes,useNpm,packageManager:pkgMgr,list,configDir:userSpecifiedConfigDir,renderer:rendererPackage,skipInstall}={})=>{var _a;if(list)return logAvailableMigrations(),null;useNpm&&(useNpmWarning(),pkgMgr="npm");let fixes2=fixId2?fixes.filter(f=>f.id===fixId2):fixes;if(fixId2&&fixes2.length===0)return logger18.info(`\u{1F4ED} No migrations found for ${import_chalk21.default.magenta(fixId2)}.`),logAvailableMigrations(),null;augmentLogsToFile();let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),{configDir:inferredConfigDir,mainConfig:mainConfigPath,version:storybookVersion}=(0,import_core_common14.getStorybookInfo)(packageManager.retrievePackageJson(),userSpecifiedConfigDir);if(!(storybookVersion&&((_a=import_semver18.default.coerce(storybookVersion))==null?void 0:_a.version)))return logger18.info(import_ts_dedent22.default`
      [Storybook automigrate] ❌ Unable to determine storybook version  so the automigrations will be skipped.
        🤔 Are you running automigrate from your project directory? Please specify your Storybook config directory with the --config-dir flag.
      `),{preCheckFailure:"undetected_sb_version"};let configDir=userSpecifiedConfigDir||inferredConfigDir||".storybook";try{await(0,import_core_common14.loadMainConfig)({configDir})}catch(err){return err.message.includes("No configuration files have been found")?(logger18.info(import_ts_dedent22.default`[Storybook automigrate] Could not find or evaluate your Storybook main.js config directory at ${import_chalk21.default.blue(configDir)} so the automigrations will be skipped. You might be running this command in a monorepo or a non-standard project structure. If that is the case, please rerun this command by specifying the path to your Storybook config directory via the --config-dir option.`),{preCheckFailure:"mainjs_not_found"}):(logger18.info(import_ts_dedent22.default`[Storybook automigrate] ❌ Failed trying to evaluate ${import_chalk21.default.blue(mainConfigPath)} with the following error: ${err.message}`),logger18.info("Please fix the error and try again."),{preCheckFailure:"mainjs_evaluation_error"})}logger18.info("\u{1F50E} checking possible migrations..");let fixResults={},fixSummary={succeeded:[],failed:{},manual:[],skipped:[]};for(let i=0;i<fixes2.length;i+=1){let f=fixes2[i],result;try{result=await f.check({packageManager,configDir,rendererPackage})}catch(error){logger18.info(`\u26A0\uFE0F  failed to check fix ${import_chalk21.default.bold(f.id)}`),logger18.error(`
${error.stack}`),fixSummary.failed[f.id]=error.message,fixResults[f.id]="check_failed"}if(result){logger18.info(`
\u{1F50E} found a '${import_chalk21.default.cyan(f.id)}' migration:`);let message=f.prompt(result);logger18.info((0,import_boxen.default)(message,{borderStyle:"round",padding:1,borderColor:"#F1618C",title:f.promptOnly?"Manual migration detected":"Automigration detected"}));let runAnswer;try{if(dryRun)runAnswer={fix:!1};else if(yes)runAnswer={fix:!0},f.promptOnly&&(fixResults[f.id]="manual_succeeded",fixSummary.manual.push(f.id));else if(f.promptOnly){fixResults[f.id]="manual_succeeded",fixSummary.manual.push(f.id),logger18.info();let{shouldContinue}=await(0,import_prompts4.default)({type:"toggle",name:"shouldContinue",message:"Select continue once you have made the required changes, or quit to exit the migration process",initial:!0,active:"continue",inactive:"quit"},{onCancel:()=>{throw new Error}});if(!shouldContinue){fixResults[f.id]="manual_skipped";break}}else runAnswer=await(0,import_prompts4.default)({type:"confirm",name:"fix",message:`Do you want to run the '${import_chalk21.default.cyan(f.id)}' migration on your project?`,initial:!0},{onCancel:()=>{throw new Error}})}catch{break}if(!f.promptOnly)if(runAnswer.fix)try{await f.run({result,packageManager,dryRun,mainConfigPath,skipInstall}),logger18.info(`\u2705 ran ${import_chalk21.default.cyan(f.id)} migration`),fixResults[f.id]="succeeded",fixSummary.succeeded.push(f.id)}catch(error){fixResults[f.id]="failed",fixSummary.failed[f.id]=error.message,logger18.info(`\u274C error when running ${import_chalk21.default.cyan(f.id)} migration`),logger18.info(error),logger18.info()}else fixResults[f.id]="skipped",fixSummary.skipped.push(f.id)}else fixResults[f.id]=fixResults[f.id]||"unnecessary"}return Object.values(fixResults).some(r=>r==="failed"||r==="check_failed")?await(0,import_fs_extra10.move)(TEMP_LOG_FILE_PATH,(0,import_path12.join)(process.cwd(),LOG_FILE_NAME),{overwrite:!0}):await(0,import_fs_extra10.remove)(TEMP_LOG_FILE_PATH),logger18.info(),logger18.info(getMigrationSummary(fixResults,fixSummary,LOG_FILE_PATH)),logger18.info(),cleanup(),fixResults};function getMigrationSummary(fixResults,fixSummary,logFile){let hasNoFixes=Object.values(fixResults).every(r=>r==="unnecessary"),hasFailures=Object.values(fixResults).some(r=>r==="failed"||r==="check_failed"),title=hasNoFixes?"No migrations were applicable to your project":hasFailures?"Migration check ran with failures":"Migration check ran successfully",successfulFixesMessage=fixSummary.succeeded.length>0?`
      ${import_chalk21.default.bold("Successful migrations:")}

 ${fixSummary.succeeded.map(m=>import_chalk21.default.green(m)).join(", ")}
    `:"",failedFixesMessage=Object.keys(fixSummary.failed).length>0?`
    ${import_chalk21.default.bold("Failed migrations:")}
 ${Object.entries(fixSummary.failed).reduce((acc,[id,error])=>`${acc}
${import_chalk21.default.redBright(id)}:
${error}
`,"")}
    
You can find the full logs in ${import_chalk21.default.cyan(logFile)}
`:"",manualFixesMessage=fixSummary.manual.length>0?`
      ${import_chalk21.default.bold("Manual migrations:")}

 ${fixSummary.manual.map(m=>fixResults[m]==="manual_succeeded"?import_chalk21.default.green(m):import_chalk21.default.blue(m)).join(", ")}
    `:"",skippedFixesMessage=fixSummary.skipped.length>0?`
      ${import_chalk21.default.bold("Skipped migrations:")}

 ${fixSummary.skipped.map(m=>import_chalk21.default.cyan(m)).join(", ")}
    `:"",summaryMessage=import_ts_dedent22.default`
    ${successfulFixesMessage}${manualFixesMessage}${failedFixesMessage}${skippedFixesMessage}${hasNoFixes?"":`
\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500

`}If you'd like to run the migrations again, you can do so by running '${import_chalk21.default.cyan("npx storybook@next automigrate")}'
    
    The automigrations try to migrate common patterns in your project, but might not contain everything needed to migrate to the latest version of Storybook.
    
    Please check the changelog and migration guide for manual migrations and more information: ${import_chalk21.default.yellow("https://storybook.js.org/migration-guides/7.0")}
    And reach out on Discord if you need help: ${import_chalk21.default.yellow("https://discord.gg/storybook")}
  `;return(0,import_boxen.default)(summaryMessage,{borderStyle:"round",padding:1,title,borderColor:hasFailures?"red":"green"})}var logger19=console,installStorybook=(projectType,packageManager,options)=>{let npmOptions={installAsDevDependencies:!0,skipInstall:options.skipInstall},packageJson;try{packageJson=packageManager.readPackageJson()}catch{}let language=detectLanguage(packageJson),pnp=detectPnp(),generatorOptions={language,builder:options.builder||detectBuilder(packageManager,projectType),linkable:!!options.linkable,pnp:pnp||options.usePnp};return(async()=>{switch(projectType){case"REACT_SCRIPTS":return REACT_SCRIPTS_default(packageManager,npmOptions,generatorOptions).then(commandLog('Adding Storybook support to your "Create React App" based project'));case"REACT":return REACT_default(packageManager,npmOptions,generatorOptions).then(commandLog(`Adding Storybook support to your "React" app
...
`),codeLog(["sb init [options] -f"]),logger19.log();return}let installResult=await installStorybook(projectType,packageManager,options).catch(e=>{process.exit()});!options.skipInstall&&!storybookInstalled&&packageManager.installDependencies(),options.disableTelemetry||(0,import_telemetry.telemetry)("init",{projectType}),projectType!=="REACT_NATIVE"&&await automigrate({yes:options.yes||process.env.CI==="true",packageManager:pkgMgr}),logger19.log(`
For more information visit:`,import_chalk22.default.cyan("https://storybook.js.org")),projectType==="ANGULAR"?(logger19.log(`
To run your Storybook, type:
`),codeLog([`ng run ${installResult.projectName}:storybook`])):projectType==="REACT_NATIVE"?(logger19.log(),logger19.log(import_chalk22.default.yellow(`NOTE: installation is not 100% automated.
...
`),logger19.log(import_chalk22.default.cyan("https://github.com/storybookjs/react-native")),logger19.log()):(logger19.log(`
To run your Storybook, type:
`),codeLog([packageManager.getRunStorybookCommand()])),logger19.log()}async function initiate(options,pkg2){await(0,import_core_server.withTelemetry)("init",{cliOptions:options},()=>doInitiate(options,pkg2))}var import_path13=__toESM(require("path")),import_fs7=__toESM(require("fs")),import_cross_spawn3=require("cross-spawn"),import_core_common15=require("@storybook/core-common"),import_csf_tools8=require("@storybook/csf-tools");var logger20=console,LEGACY_CONFIGS=["addons","config","presets"],postinstallAddon=async(addonName,isOfficialAddon)=>{let skipMsg=null;isOfficialAddon?import_fs7.default.existsSync(".storybook")?(skipMsg="no codmods found",LEGACY_CONFIGS.forEach(config=>{try{let codemod=require.resolve(`${getPackageName(addonName,isOfficialAddon)}/postinstall/${config}.js`);commandLog(`Running postinstall script for ${addonName}`)();let configFile=import_path13.default.join(".storybook",`${config}.ts`);import_fs7.default.existsSync(configFile)||(configFile=import_path13.default.join(".storybook",`${config}.js`),import_fs7.default.existsSync(configFile)||import_fs7.default.writeFileSync(configFile,"","utf8")),(0,import_cross_spawn3.sync)("npx",["jscodeshift","-t",codemod,configFile],{stdio:"inherit",shell:!0}),skipMsg=null}catch{}})):skipMsg="no .storybook config":skipMsg="unofficial addon",skipMsg&&commandLog(`Skipping postinstall for ${addonName}, ${skipMsg}`)()},getVersionSpecifier=addon=>{let groups=/^(...*)@(.*)$/.exec(addon);return groups?[groups[1],groups[2]]:[addon,void 0]};async function add(addon,options){let{packageManager:pkgMgr}=options;options.useNpm&&(useNpmWarning(),pkgMgr="npm");let packageManager=JsPackageManagerFactory.getPackageManager({force:pkgMgr}),packageJson=packageManager.retrievePackageJson(),[addonName,versionSpecifier]=getVersionSpecifier(addon),{mainConfig,version:storybookVersion}=(0,import_core_common15.getStorybookInfo)(packageJson);if(!mainConfig){logger20.error("Unable to find storybook main.js config");return}let main=await(0,import_csf_tools8.readConfig)(mainConfig),addons=main.getFieldValue(["addons"]);addons&&!Array.isArray(addons)&&logger20.error("Expected addons array in main.js config"),logger20.log(`Verifying ${addonName}`);let latestVersion=packageManager.latestVersion(addonName);latestVersion||logger20.error(`Unknown addon ${addonName}`);let isStorybookAddon=addonName.startsWith("@storybook/"),addonWithVersion=`${addonName}@${versionSpecifier||(isStorybookAddon?storybookVersion:latestVersion)}`;logger20.log(`Installing ${addonWithVersion}`),packageManager.addDependencies({installAsDevDependencies:!0},[addonWithVersion]),logger20.log(`Adding '${addon}' to main.js addons field.`);let updatedAddons=[...addons||[],addonName];main.setFieldValue(["addons"],updatedAddons),await(0,import_csf_tools8.writeConfig)(main),options.skipPostinstall||await postinstallAddon(addon,isStorybookAddon)}var import_codemod=require("@storybook/codemod");async function migrate(migration,{glob,dryRun,list,rename,logger:logger28,parser}){if(list)(0,import_codemod.listCodemods)().forEach(key=>logger28.log(key));else if(migration)await(0,import_codemod.runCodemod)(migration,{glob,dryRun,logger:logger28,rename,parser});else throw new Error("Migrate: please specify a migration name or --list")}var import_path14=__toESM(require("path")),import_fs_extra11=require("fs-extra"),import_puppeteer_core=__toESM(require("puppeteer-core")),import_express=__toESM(require("express")),import_get_port=__toESM(require("get-port")),import_node_logger3=require("@storybook/node-logger"),read=async url=>{let browser=await usePuppeteerBrowser(),page=await browser.newPage();await page.goto(url),await page.waitForFunction(`
