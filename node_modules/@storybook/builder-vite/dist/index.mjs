...
`)}])
    ...
    `.trim()}import*as path2 from"path";import{normalizePath as normalizePath2}from"vite";import{logger}from"@storybook/node-logger";import*as posixPath from"path/posix";import{promise as glob}from"glob-promise";import{normalizeStories as normalizeStories2}from"@storybook/core-common";async function listStories(options){return(await Promise.all(normalizeStories2(await options.presets.apply("stories",[],options),{configDir:options.configDir,workingDir:options.configDir}).map(({directory,files})=>{let pattern=posixPath.join(directory,files);return glob(posixPath.isAbsolute(pattern)?pattern:posixPath.join(options.configDir,pattern),{follow:!0})}))).reduce((carry,stories)=>carry.concat(stories),[])}function toImportPath(relativePath){return relativePath.startsWith("../")?relativePath:`./${relativePath}`}async function toImportFn(stories){return`
    const importers = {
      ${stories.map(file=>{let ext=path2.extname(file),relativePath=normalizePath2(path2.relative(process.cwd(),file));return[".js",".jsx",".ts",".tsx",".mdx",".svelte"].includes(ext)||logger.warn(`Cannot process ${ext} file with storyStoreV7: ${relativePath}`),`  '${toImportPath(relativePath)}': async () => import('/@fs/${file}')`}).join(`,
`)}
    };
